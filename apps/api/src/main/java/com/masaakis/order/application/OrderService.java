...
public class OrderService {
    ...
    public TableMenu tableMenu(TableSessionService.TableContext context){
        ...
        return new TableMenu(context.tableName(),context.areaName(),menus.getPublishedMenu(slug),entitlements.active(context.tenantId(),"TABLE_ORDERING"));
    }

    @Transactional
    public OrderView create(TableSessionService.TableContext context,String idempotencyKey,List<CreateItem> requested,String note){
        ...
        UUID orderId=UUID.randomUUID();int inserted=jdbc.sql("""
                insert into customer_order(id,tenant_id,branch_id,table_id,table_session_id,service_mode,state,idempotency_key,estimated_total,currency,customer_note)
                values(:id,:tenantId,:branchId,:tableId,:sessionId,'DINE_IN','SUBMITTED',:key,:total,:currency,:note)
                ...
                .param("total",total).param("currency",currency).param("note",clean(note,500)).update();
    ...
    @RequiresAddon("TABLE_ORDERING") @Transactional(readOnly=true)
    ...
    public OrderView transition(StaffPrincipal p,UUID id,String target,long version){
        ...
        requireTransition(p,current,target);int changed=jdbc.sql("""
    ...
    private Optional<OrderView> order(UUID id,UUID tenantId,String scope,Map<String,Object> params){
        var statement=jdbc.sql("""
                select o.id,o.table_id,t.name table_name,o.service_mode,o.state,o.estimated_total,o.currency,o.customer_note,o.version,o.submitted_at
        ...
        var base=statement.query((rs,n)->new OrderBase(rs.getObject("id",UUID.class),rs.getObject("table_id",UUID.class),rs.getString("table_name"),
                ...
                rs.getString("customer_note"),rs.getLong("version"),rs.getObject("submitted_at",java.time.OffsetDateTime.class))).optional();
        return base.map(o->new OrderView(o.id,o.tableId,o.tableName,o.serviceMode,o.state,o.total,o.currency,o.note,o.version,o.submittedAt,
    ...
    private void requireTransition(StaffPrincipal p,String from,String to){Set<String> allowed=switch(from){case"SUBMITTED"->Set.of("ACCEPTED","REJECTED","CANCELLED");case"ACCEPTED"->Set.of("PREPARING","CANCELLED");case"PREPARING"->Set.of("READY");case"READY"->Set.of("SERVING");case"SERVING"->Set.of("DELIVERED");default->Set.of();};
        if(!allowed.contains(to))throw new AppException(HttpStatus.CONFLICT,"ORDER_TRANSITION_INVALID","Sipariş durum geçişi geçersiz.");
        if(Set.of("ACCEPTED","REJECTED","CANCELLED").contains(to))p.require("order/accept-reject");else if(Set.of("PREPARING","READY").contains(to))p.require("order/prepare-ready");else p.require("order/deliver");}
    ...
    private record PricedItem(UUID id,String name,BigDecimal price,String currency,int quantity,String notes){}private record OrderBase(UUID id,UUID tableId,String tableName,String serviceMode,String state,BigDecimal total,String currency,String note,long version,java.time.OffsetDateTime submittedAt){}
    ...
    public record OrderView(UUID id,UUID tableId,String tableName,String serviceMode,String state,BigDecimal estimatedTotal,String currency,String customerNote,long version,java.time.OffsetDateTime submittedAt,List<OrderItemView>items){}
    public record TableMenu(String tableName,String areaName,PublicMenuResponse menu,boolean orderingEnabled){}
